class SendOrderJob < ApplicationJob
  ...
  def write_order_sheet_to_send
    ...
    google_drive_client.move_file(spreadsheet, Google.orders_to_send_folder_id)
  ...
  def send_order_sheet
    google_gmail_client.send_email(
      Google.my_email,
      Google.my_email,
      Google.order_subject,
      Google.order_body,
      order_sheet_download,
      Google.order_file_name_excel
    )
  ...
  def save_order_in_sheets_database
    ...
    last_row = google_sheets_client.read_last_row(Google.orders_database_spreadsheet_id, 'Pedidos')
    last_order_id = last_row ? last_row[0].to_i : 0
    new_order_id = last_order_id + 1

    last_row_index = last_row ? last_row_index = google_sheets_client.get_last_row_index(Google.orders_database_spreadsheet_id, 'Pedidos') : 2
    ...
    google_sheets_client.write_to_sheet(Google.orders_database_spreadsheet_id, "Pedidos!A#{start_row}:Q#{start_row + rows.size - 1}", rows)
